class WebSocketV1Transport {
  ...
  connect() {
    ...
    this.ws.onmessage = function (wsmsg) {
      let pausePlay = document.getElementById('isRunLive');

      if (pausePlay.checked) {
        let data = [];
        try {
        ...
        let optionList = hostSelect.options;
        let hostS = [...optionList].map((item) => {
          return item.value;
        });
        if (typeof data[1].host_name != 'undefined' && !hostS.includes(data[1].host_name)) {
          let opt = document.createElement('option');
          opt.value = data[1]['host_name'];
          opt.innerHTML = data[1].host_name;
        ...
        if (wsMsgType === WebSocketV1Transport.MSG_PING) {
          ...
          if (typeof data[1].host_name == 'undefined' || hostSelect.options[hostSelect.selectedIndex].value == data[1].host_name) {
            if (typeof data[1].header != 'undefined' && data[1].header.frame_id == "odom") {
              State.insert('robotLocation', data[1].pose.pose.position);
            }
            if (typeof data[1].header != 'undefined' && data[1].header.frame_id == "array") {
              const labels = [];
              for (let i = 0; i < data[1].layout.dim.length; i++) {
                labels.push(data[1].layout.dim[i].label);
              }
              State.insert('arrayLabels', labels);
            }
            that.onMsg(data[1]);
          }
        }
